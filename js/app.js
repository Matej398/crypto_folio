...
function renderHistoryList() {
    ...
    const markup = activeHistory.map((entry, index) => {
        ...
        const notesCount = Array.isArray(entry.notes) ? entry.notes.length : 0;
            ...
            <article class="history-card ${isExpanded ? 'expanded' : ''}" data-index="${index}">
                <div class="history-card-header">
                    <div class="history-card-meta">
                        ${hasNotes ? `
                            <div class="history-notes-indicator" title="${notesCount} note${notesCount !== 1 ? 's' : ''}">
                                ...
                                ${notesCount > 0 ? `<span class="history-notes-badge">${notesCount > 99 ? '99+' : notesCount}</span>` : ''}
                            </div>
                        ` : ''}
