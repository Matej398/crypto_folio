...
-- Add avatar_url if missing (only if it doesn't exist)
...
-- Add api_usage_data if missing (only if it doesn't exist)
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = 'crypto_portfolio' 
...
SET @sql = IF(@col_exists = 0, 
    ...
    'SELECT "Column api_usage_data already exists" AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- Add notes column if missing (only if it doesn't exist)
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = 'crypto_portfolio' 
...
-- Add fear_greed_index column if missing (only if it doesn't exist)
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = 'crypto_portfolio' 
    AND TABLE_NAME = 'portfolio_history' 
...
SET @sql = IF(@col_exists = 0, 
    ...
    'SELECT "Column fear_greed_index already exists" AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
